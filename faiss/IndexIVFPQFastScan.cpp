        ...
        if (n < 2) {
            ...
            if (impl == 14 || impl == 15 ) {
                if (by_residual && metric_type == METRIC_L2) {
                    ...
                    nslice = std::max(size_t(n / nq_ok), size_t(1));
                } else {
                    nslice = 1;
                }
            } else {
            ...
            // impl 14 and 15 are internally threaded but they might require splitting by queries if LUT is too big
            if (impl == 14 || impl == 15 ) {
...
template <class C>
void IndexIVFPQFastScan::search_implem_14(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
    ...
    size_t nlist_visited = 0;
        ...
        uint64_t ttg3_b = get_cy();
        uint64_t fn_tt = ttg3_b - ttg3;

#pragma omp parallel  reduction(+ : ndis, nlist_visited)
    {   
        uint64_t ttg3_c = get_cy();
        uint64_t thread_cr_tt = ttg3_c - ttg3_b;

        std::vector<idx_t> local_idx(k * n);
        std::vector<float> local_dis(k * n);
        uint64_t ttg4 = get_cy();
        uint64_t local_mem_tt = ttg4 - ttg3_c;
